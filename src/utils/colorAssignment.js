 ...
 * 
 * @param {string[] | {name: string, color: string}[]} participants - 참가자 배열
 * @returns {{name: string, color: string}[]} 정규화된 참가자 배열
 */
export function normalizeParticipants(participants) {
  ...
  if (participants.length > 0 && typeof participants[0] === 'object' && participants[0].name) {
    // 색상이 없는 참가자에게 색상 할당
    return participants.map((p, index) => {
      ...
      const usedColors = participants
        .filter((prev, i) => i < index && prev.color)
        .map(prev => prev.color);
 ...
 * 
 * @param {string[] | {name: string, color: string}[]} participants - 참가자 배열
 * @param {number} index - 현재 참가자의 인덱스
 * @returns {string} 할당된 색상 (HEX 코드)
 */
export function assignColor(participants, index) {
  // 기존 참가자들의 색상 추출
  const usedColors = participants
    .filter((p, i) => i < index && typeof p === 'object' && p.color)
    .map((p) => p.color);
 ...
 * 
 * @param {{name: string, color: string}[]} existingParticipants - 기존 참가자 배열
 * @returns {string} 할당된 색상 (HEX 코드)
 */
export function assignColorToNewParticipant(existingParticipants) {
